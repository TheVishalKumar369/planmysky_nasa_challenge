...
class HistoricalWeatherPredictor:
    ...
    def __init__(self, data_dir: str = "../../data/processed", location_name: str = None):
        ...
        try:
            ...
            if self.location_name and self.location_name != "Unknown":
                ...
                if self.location_folder.exists():
                    ...
                    parquet_files = list(self.location_folder.glob("era5_processed_*.parquet"))
                    if parquet_files:
                        logger.info(f"Loading {parquet_files[0].name}...")
                        self.df = pd.read_parquet(parquet_files[0])
                    else:
                        # Fall back to CSV
                        csv_files = list(self.location_folder.glob("era5_processed_*.csv"))
                        if csv_files:
                            logger.info(f"Loading {csv_files[0].name}...")
                            self.df = pd.read_csv(csv_files[0])
                        else:
                            raise FileNotFoundError(f"No processed data files found in {self.location_folder}")
                else:
                    raise FileNotFoundError(f"Location folder not found: {self.location_folder}")
            else:
                # Legacy: search in root directory
                parquet_files = list(self.data_dir.glob("era5_processed_*.parquet"))
                if parquet_files:
                    logger.info(f"Loading {parquet_files[0].name}...")
                    self.df = pd.read_parquet(parquet_files[0])
                else:
                    # Fall back to CSV
                    csv_files = list(self.data_dir.glob("era5_processed_*.csv"))
                    if csv_files:
                        logger.info(f"Loading {csv_files[0].name}...")
                        self.df = pd.read_csv(csv_files[0])
                    else:
        ...
        try:
            ...
            try:
                ...
                if future_date.date() < today.date():
                    future_date = datetime(current_year + 1, target_month, target_day)
            except ValueError:
            ...
            target_date = pd.Timestamp(year=2000, month=target_month, day=target_day)
            ...
            logger.info(f"Found {len(matches)} historical records across {n_years} years")
            ...
            light_rain = ((matches['PRECTOT_mm'] > 0.1) & (matches['PRECTOT_mm'] <= 2.5)).sum()
            moderate_rain = ((matches['PRECTOT_mm'] > 2.5) & (matches['PRECTOT_mm'] <= 10)).sum()
            ...
            # ===== ADDITIONAL FEATURES =====
            humidity_mean = float(matches['D2M'].mean()) if 'D2M' in matches.columns else None
            pressure_mean = float(matches['MSL'].mean()) if 'MSL' in matches.columns else None
            solar_mean = float(matches['SSRD'].mean()) if 'SSRD' in matches.columns else None
            ...
            result = {
                # Metadata
                "date": future_date_str,  # Next future occurrence (e.g., "2026-07-15")
                "month_day": f"{target_month:02d}-{target_day:02d}",  # For reference
            ...
            logger.error(f"Error predicting for date {target_month}-{target_day}: {e}")
            return {"error": str(e)}
...
def main():
    ...
    if choice == "1":
        ...
        print("This includes all occurrences of this date across all years in the dataset.\n")
        ...
        print(f"\nRAINFALL:")
        print(f"  Probability: {prediction['rainfall']['probability_percent']}%")
        print(f"  Expected amount: {prediction['rainfall']['expected_amount_mm']} mm")
        print(f"  Max recorded: {prediction['rainfall']['max_recorded_mm']} mm")

        print(f"\nTEMPERATURE:")
        print(f"  Expected: {prediction['temperature']['expected_range']['min']}C - {prediction['temperature']['expected_range']['max']}C")
        print(f"  Average mean: {prediction['temperature']['mean_avg_celsius']}C")
        ...
        print(f"\nEXTREME WEATHER PROBABILITIES:")
        print(f"  Temperature > 30C: {prediction['extreme_probabilities']['temp_above_30C']*100:.1f}%")
        print(f"  Heavy rain > 10mm: {prediction['extreme_probabilities']['heavy_rain_above_10mm']*100:.1f}%")
        ...
        output_path = output_location_dir / f"prediction_{month:02d}_{day:02d}.json"
        ...
        print(f"Average monthly rainfall: {stats['rainfall']['average_monthly_total_mm']} mm")
        print(f"Average temperature: {stats['temperature']['average_mean_celsius']}°C")
